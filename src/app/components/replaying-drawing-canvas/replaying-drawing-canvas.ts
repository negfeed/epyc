...
export class ReplayingDrawingCanvas extends DrawingCanvas implements OnDestroy {
  ...
  private normalizeDrawingEventsTimestamps() {
    const processedDrawingEvents: Array<DrawingEvent> = [];
    this.drawingEvents.forEach(
      (drawingEvent: DrawingEvent, index: number, drawingEvents: Array<DrawingEvent>) => {
        ...
        if (index === 0) {
          ...
          processedDrawingEvents[index].timestamp =
            processedDrawingEvents[index - 1].timestamp + timestampDifference;
        }
      },
    );
  ...
  private initializeDrawingState(drawingModelInstance: DrawingModelInterface) {
    this.drawingEvents =
      drawingModelInstance && drawingModelInstance.drawingEvents
        ? Object.keys(drawingModelInstance.drawingEvents).map(
  ...
  private processNextDrawingPeriod() {
    const currentTimestamp = this.replayPeriodCounter * this.REPLAY_PERIOD_IN_MILLIS;
    ...
    setTimeout(() => this.processNextDrawingPeriod(), this.REPLAY_PERIOD_IN_MILLIS);
  }
